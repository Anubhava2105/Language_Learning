...
const Notice = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        const response = await axios.post(
          ...
          { type: data.language },  // Pass user's language to backend
          { headers: headers }
        );
        if (response.data.success) {
  ...
  const addNoticeHandler = (e) => {
    ...
    // Modify the data object to use 'language' as 'type'
    const noticeData = {
      title: data.title,
      description: data.description,
      type: data.language, // Use 'language' as 'type'
      link: data.link
    };
  
    axios
      .post(`${baseApiURL()}/notice/addNotice`, noticeData, { headers: headers })
      .then((response) => {
        toast.dismiss();
        if (response.data.success) {
